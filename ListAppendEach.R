ListAppendEach <- function(x, values) {
    ...
    # example: AppendEach(NULL, list(a=1,b=2)) --> list(a=1,b=2)
    # example: AppendEach(list(a=1,b=2), list(a=10,b=20)) --> list(a=list(1,10), b=list(2,20))
    if (is.null(x)) {
        values
    } else {
...
ListAppendEach.test <- function() {
    ...
    v1 <- list(a=1, b=2)
    v2 <- list(a=10, b=20)
    xx <- ListAppendEach(LisppendEach(x, v1), v2)
    n <- names(xx)
    stopifnot(n[1] == 'a')
    stopifnot(n[2] == 'b')
