...
local function getWeights(trainingLocations,
                          ...
                          hp,
                          names)
   local vp, verbose = makeVp(2, 'getWeights')
   ...
   if debugging then
      ...
      local cpuOverall = timerOverall:cpu()
      vp(2, 
...
local function validationError(train,
   ...
   local function getPredictionActual(i)
      local vp = makeVp(2, 'getPrediction')
      vp(1, 'i', i)
      local predictionActual = tableCached:fetch(i)
      local timer = Timer()
      local fetched = cf:fetch{keys={i}} -- fetch returns a sequence or nil
      local prediction, actual
      if predictionActual ~= nil then
         prediction = predictionActual[1]
         actual = predictionActual[2]
      else
         local timerWeights = Timer()
         ...
         vp(2, 'i', i, 'weightCpu', weightCpu, 'llrCpu', llrCpu)
         ...
         if i % cacheWriteFrequency == 0 then
            vp('writing predictions to ' .. cachePath)
            tableCached:writeToFile()
         end
      end
      local cpu = timer:cpu()
      local wallclock = timer:wallclock() 
      if cpu > wallclock then
   ...
   for i = 1, nValObservations do
      local prediction, cpuSecs, wallclockSecs = getPrediction(i)
      local actual = val.targets.t[i][1]
      vp(1, string.format('%d of %d prediction %d actual %d cpu %f7.4 wallclock %f7.4',
...
local fTrain = .60
local fValidate = .20
local train, val, test = splitParse(labeled, fTrain, fValidate, var) -- creates NamedMatrices
