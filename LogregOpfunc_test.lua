...
local function _structureTheta_test() -- old API
...
local function _structureTheta_test() -- new API
   local of = makeOfFromTestExample()
   local theta = testExample.theta
...
local function timingOverall(nIterations)
   ...
   local function lossGradient(theta)
      local loss, info = of:loss(theta)
      local gradient = of:gradient(theta, info)
...
if false then
   ...
   print('you skipped timing of _scores implemenations')
end
...
if true then
   ...
   print('you skipped timing of gradient implementations')
end
