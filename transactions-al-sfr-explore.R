...
ReadDeeds <- function(control) {
    ...
    only.deed.fields
    # Print data for special APNs
    cat('in ReadDeeds', nrow(deeds), '\n'); browser()
    special.apns <- c(5517010009, 5033011010, 5043006019)
    apn.unformatted <- as.numeric(only.deed.fields$APN.UNFORMATTED)
    PrintData <- function(special.apn) {
        cat('start PrintData', special.apn, '\n'); browser()
        has.apn <- only.deed.fields[ apn.unformatted == special.apn
                                    ,c( 'APN.UNFORMATTED'
                                       ,'SALE.DATE'
                                       ,'SALE.AMOUNT'
                                       ,'DOCUMENT.TYPE.CODE'    # DEEDC?
                                       ,'TRANSACTION.TYPE.CODE' # TRNTP
                                       ,'SALE.CODE'             # SCODE
                                       )
                                    ]
        print(has.apn)
    }
    Map(PrintData, special.apns)
