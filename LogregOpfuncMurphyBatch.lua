...
-- ref: Murphy, p. 253
function LogregOpfuncMurphyBatch:_gradient_ixxxx(i, lossInfo, implementation)
   implementation = implementation or 1
   if implementation == 1 then
...
function LogregOpfuncMurphyBatch:_gradient_i_implementation_1(i, lossInfo)
...
function LogregOpfuncMurphyBatch:_gradient_i_implementation_2(i, lossInfo)
   local mu = lossInfo.probs[i]
   local y = torch.Tensor(self.nClasses):zero()
   y[self.y[i]] = 1
   local errors = (mu - y) * self.s[i]
...
function LogregOpfuncMurphyBatch:_gradient_i_implementation_3(i, lossInfo)
...
-- RESULT: this implementation takes ? times as long as implementation 1
function LogregOpfuncMurphyBatch:_gradient_i_implementation_4(i, lossInfo)
   local vp = makeVp(2, 'implementation 3')
