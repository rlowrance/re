...
function readfeature(featurename)
   ...
   for line in io.lines(ucladir .. "data.csv") do
      local record = fromCSV(line)
      if firstline then
	 firstline = false           -- skip header
	 featureindex = indexof(featurename, record)
      else 
	 res[#res+1] = record[featureindex] + 0 -- convert to number
      end
   end
   return res
end
...
mlogitfile = io.input(ucladir .. "mlogit-summary.txt")
